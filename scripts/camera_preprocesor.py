...
class DnnPreprocessNode(object):
	def __init__(self):
		...
		self.image_pub = rospy.Publisher("/mast_cam/cropped/compressed", CompressedImage, queue_size=1)
		self.info_pub = rospy.Publisher("/mast_cam/cropped/camera_info", CameraInfo, queue_size=1)
		self.image_sub = rospy.Subscriber("/mast_cam/image_rect/compressed", CompressedImage, self.image_cb, queue_size=1, buff_size=2 ** 24)
		self.info_sub = rospy.Subscriber("/mast_cam/image_rect/camera_info", CameraInfo, self.info_cb, queue_size=1)
	...
	def image_cb(self, msg):
		...
		out_msg = self.bridge.cv2_to_compressed_imgmsg(cropped, dst_format="jpg")
